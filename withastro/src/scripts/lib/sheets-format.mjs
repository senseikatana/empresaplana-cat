...
export function parseSpreadsheet(tabs) {
	...
	for (const tab of tabs) {
		...
		const rows = (tab.values ?? []).filter((row) =>
	...
	const allIds = new Set([
		...metadata.keys(),
		...schedules.map((s) => s.lineId),
	]);
	for (const lineId of allIds) {
		const meta = metadata.get(lineId);
		const tabName = tabTitleById.has(lineId)
			? String(tabTitleById.get(lineId)).replace(/^\d+\s*[-–—]\s*/, "")
			: "";
		lines.set(lineId, {
			name: meta?.name || tabName || `Línea ${lineId}`,
			pdfUrl: meta?.pdfUrl ?? "",
		});
	}
...
export function buildSpreadsheet(lineRows, schedulesByLine, connections = []) {
	const out = {
		[LINEAS_TAB]: [
			["id", "nombre", "pdf_url"],
			...lineRows.map((l) => [l.id, l.name, l.pdfUrl ?? ""]),
		],
		[CONEXIONES_TAB]: [
	...
	for (const line of lineRows) {
		...
		const keyToIndex = new Map(
			header.map((s, i) => [stopKey(s.town, s.name), i]),
		);
