...
function findStopIndex(stops: Stop[], label: string): number {
	...
	return stops.findIndex(
		(stop) => normalize(`${stop.town} — ${stop.name}`) === normLabel,
	);
}
...
function matchLeg(stops: Stop[], normOrigin: string, normDest: string) {
	...
	const idxD = stops.findIndex(
		(stop, i) => i > idxO && normalize(stop.town) === normDest,
	);
...
export async function searchRoutes(
	...
	const direct = [...directMap.values()].sort((a, b) =>
	...
	if (withTransfers && normOrigin !== normDest) {
		...
		const hubsFromOrigin = new Map<
			string,
			{ schedule: LoadedSchedule; idxO: number }[]
		>();
		const hubsToDestination = new Map<
			string,
			{ schedule: LoadedSchedule; idxD: number }[]
		>();

		for (const schedule of loaded) {
			if (matchLeg(schedule.stops, normOrigin, normDest)) continue;
			const idxO = schedule.stops.findIndex(
				(stop) => normalize(stop.town) === normOrigin,
			);
			if (idxO !== -1) {
				const hubs = [
					...new Set(
						schedule.stops.slice(idxO + 1).map((stop) => normalize(stop.town)),
					),
				];
			...
			const idxD = schedule.stops.findIndex(
				(stop) => normalize(stop.town) === normDest,
			);
			if (idxD !== -1) {
				const hubs = [
					...new Set(
						schedule.stops.slice(0, idxD).map((stop) => normalize(stop.town)),
					),
				];
				for (const hub of hubs) {
		...
		for (const [hub, fromList] of hubsFromOrigin) {
			...
			const hubIdxA = legA.schedule.stops.findIndex(
				(stop, i) => i > legA.idxO && normalize(stop.town) === hub,
			);
			const hubIdxB = legB.schedule.stops.findIndex(
				(stop, i) => i < legB.idxD && normalize(stop.town) === hub,
			);
			...
			transfers.push({
				...
				firstLeg: buildSummary(
					...
					[
						{
							departureTime: legADepartures[0].time,
							arrivalTime: legADepartures[legADepartures.length - 1].time,
						},
					],
				...
				secondLeg: buildSummary(
					...
					[
						{
							departureTime: legBDepartures[0].time,
							arrivalTime: legBDepartures[legBDepartures.length - 1].time,
						},
					],
					MAX_DEPARTURES_PER_LEG,
				),
			...
			for (const conn of connections) {
				...
				for (const sA of aSchedules) {
					...
					for (const sB of bSchedules) {
						...
						if (idxSB === -1) continue;
						const idxD = sB.stops.findIndex(
							(stop, i) => i > idxSB && normalize(stop.town) === normDest,
						);
						if (idxD === -1) continue;
				...
				through.push({
					...
					firstLeg: buildSummary(
						...
						sortedPairs.map((p) => ({
							departureTime: p.firstDeparture,
							arrivalTime: p.arriveAtConnection,
						})),
						MAX_DEPARTURES_PER_LEG,
					),
					secondLeg: buildSummary(
						...
						sortedPairs.map((p) => ({
							departureTime: p.secondDeparture,
							arrivalTime: p.arriveAtDestination,
						})),
						MAX_DEPARTURES_PER_LEG,
					),
					pairs: sortedPairs,
